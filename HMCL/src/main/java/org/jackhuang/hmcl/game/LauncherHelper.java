    ...
    private final class HMCLProcessListener implements ProcessListener {
        ...
        public void onLog(String log, boolean isErrorStream) {
            ...
            if (showLogs) {
                ...
                runLater(() -> logWindow.logLine(filteredLog, level));
            }
        ...
        public void onExit(int exitCode, ExitType exitType) {
            if (showLogs) {
                runLater(() -> logWindow.logLine(String.format("[HMCL ProcessListener] Minecraft exit with code %d(0x%x), type is %s.", exitCode, exitCode, exitType), Log4jLevel.INFO));
            }
            ...
            if (exitType != ExitType.NORMAL) {
                ...
                runLater(() -> new GameCrashWindow(process, exitType, repository, version, launchOptions, pairs).show());
            }
